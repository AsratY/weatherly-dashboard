...
function updateWeatherInfo(data) {
  ...
  document.getElementById("weatherCondition").innerText = data.list[0].weather[0].description; // Set weather condition
...
function updateForecast(forecastList) {
  ...
  Object.keys(dailyForecasts).forEach((day) => {
    ...
    forecastCard.innerHTML = `
            ...
            <p align="center" style="color: hsl(258, 43%, 18%); font-weight:bold;">${forecast.weather[0].description}</p>
            <p><font color=" #261a42"><b>Temp: ${forecast.main.temp.toFixed(1)}°C</b></font></p>
            <p><font color=" #261a42"><b>Humidity: ${forecast.main.humidity}%</b></font></p>
        `;
...
function getLocationAndFetchWeather() {
  if (navigator.geolocation) {
    navigator.geolocation.getCurrentPosition(
      ...
      (error) => {
        showErrorMessage("Unable to retrieve your location. Please search manually.");
      }
    );
  } else {
